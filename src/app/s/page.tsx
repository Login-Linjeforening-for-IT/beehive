...
export default function SearchAnimationPage() {
    ...
    const [engine, setEngine] = useState<EngineKey>('brave')
    ...
    function engineName(value: EngineKey, content: typeof no) {
        if (value === 'duckduckgo') {
            return content.entry.engines.duckduckgo
        }

        if (value === 'brave') {
            return content.entry.engines.brave
        }

        return content.entry.engines.google
    }

    function stageLabel(currentStage: PlaybackStage) {
        if (currentStage === 'typing') {
            return text.entry.playback.typing
        }

        if (currentStage === 'submitting') {
            return text.entry.playback.submitting
        }

        if (currentStage === 'redirecting') {
            return `${text.entry.playback.redirecting} ${engineName(engine, text)}`
        }

        return text.entry.playback.idle
    }
    ...
    function startPlayback(nextQuery: string, nextEngine: EngineKey) {
        ...
        const writeLetter = (index: number) => {
            ...
            >
                <section
                    className='relative w-full min-h-[calc(100vh-var(--h-topbar)-1.2rem)]
                        ...
                        bg-linear-to-br from-(--color-bg-surface)
                        via-(--color-bg-body) to-(--color-bg-surface-raised)
                        p-4 shadow-[0_0.8rem_2rem_rgba(0,0,0,0.2)]
                        400px:p-6 800px:p-10'
                >
                    <div
                        aria-hidden='true'
                        className='pointer-events-none absolute -right-22 -top-22 h-64 w-64
                            rounded-full bg-(--color-primary) opacity-18 blur-3xl'
                    />
                    <div
                        aria-hidden='true'
                        className='pointer-events-none absolute -bottom-20 -left-20 h-56 w-56
                            rounded-full bg-(--color-bg-surface-raised) opacity-75 blur-3xl'
                    />
                        ...
                        >
                            <div
                                className='mx-auto w-full max-w-5xl rounded-full
                                    border border-(--color-border-default)
                                    bg-(--color-bg-surface)
                                    px-4 py-3 shadow-[0_0.5rem_1.2rem_rgba(0,0,0,0.14)]
                                    600px:px-8'
                            >
