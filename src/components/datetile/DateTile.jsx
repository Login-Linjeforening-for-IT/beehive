...
const DateTile = ({ i18n, startDate, endDate, color, varient='regular', useDayText=false }) => {
    ...
    if (ColorManipulation.isValidHex(color)) {
        if (varient === 'regular') {
            ...
            background = ColorManipulation.hexToRgba(color, 0.7);
        }
    } else {
        background = color;
    }
    ...
    if (useDayText) {
        return (
            <div className={`date-tile date-tile--${varient} ${sDate === eDate ? '' : 'date-tile--wide'}`} style={{background: background}}>
                <div className='date-tile__date'>
                    <div className='date-tile__dayofweek'>{daysOfWeek[lang][sTime.getDay()]}.</div>
                </div>
            </div>
        );
    }
    if (sMonth === eMonth) {
        return (
            <div className={`date-tile date-tile--${varient} ${sDate === eDate ? '' : 'date-tile--wide'}`} style={{background: background}}>
                <div className='date-tile__date'>
                    <div className={`date-tile__day ${sDate === eDate ? '' : 'date-tile__day--wide'}`}>{(sDate === eDate) ? sDate : sDate + '-' + eDate}</div>
        ...
        return (
            <div className={`date-tile date-tile--wide date-tile--${varient}`} style={{background: background}}>
                <div className='date-tile__date'>
                    <div className='date-tile__day date-tile__day--wide'>{sDate}</div>
                    <div className='date-tile__month date-tile__month--wide'>{months[lang][sMonth]}</div>
                </div>
                <div className='date-tile__devider'>-</div>
                <div className='date-tile__date'>
                    <div className='date-tile__day date-tile__day--wide'>{eDate}</div>
                    <div className='date-tile__month date-tile__month--wide'>{months[lang][eMonth]}</div>
                </div>
            </div>
        );
    }
