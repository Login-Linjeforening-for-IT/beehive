...
export default async function JobadsPreview() {
    ...
    return (
        <>
            <section test-id='jobs' className='pt-8 max-w-[var(--w-page)] 800px:px-4 800px:mx-auto 1000px:w-full'>
                ...
                {typeof jobads !== 'string' && Array.isArray(jobads.jobs) && jobads.jobs.length > 0 && (
                    <ul className='relative grid grid-flow-col list-none overflow-auto p-[0_1rem_1rem_1rem] snap-x snap-mandatory 400px:gap-4 800px:grid-cols-2 800px:grid-flow-row-dense 800px:gap-8 1000px:grid-cols-3 1000px:gap-4 1000px:p-0'>
                        ...
                        {jobads.jobs.map((e: any) => (
                            <li key={e.id} className='snap-center w-[80vw] max-w-88 min-w-72 800px:w-full 800px:max-w-112 1000px:m-[0_auto]'>
                                <JobadCard jobad={e} />
                            </li>
                        ))}
