...
export default function TopBar({lang, onlyLogo, token}: TopBarProps) {
    ...
    if (onlyLogo) {
        return (
            <div className={`flex w-[97.5vw] max-w-[97.5vw] m-auto p-2 h-(--h-topbar) transition duration-500 800px:justify-between 800px:p-4 ${isOpen ? 'topbar--open' : ''}`}>
                <div className='block h-12 p-[0.2rem] 800px:p-0'>
    ...
    return (
        <div className={`flex max-w-[calc(var(--w-page)+2rem)] w-full m-auto p-2 h-(--h-topbar) transition duration-500 800px:justify-between 800px:p-4 ${isOpen ? 'topbar--open' : ''}`}>
            <div className='block h-12 p-[0.2rem] 800px:p-0'>
            ...
            <nav className='flex w-[calc(100vw-8rem)] justify-end h-12 mr-4 800px:w-fit 800px:mr-0'>
                <ThemeToggle />
                <LangToggle serverLang={lang} />
                <div className='rounded-(--border-radius) hover:bg-[#6464641a] h-12 w-12'>
                    {token ?
                        <Link
